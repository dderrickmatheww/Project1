...
$(".yt-pop").on("click", function(event) {
  ...
  var url =
    ...
    input +
    "&key=AIzaSyAhsb0OUjYC9-im6U3pNoks26zkjBWUtHo";
  $("#player").empty();
  $.ajax({
    url: url,
    method: "GET"
    ...
    $("#add-vid").on("click", function(event) {
      ...
      $.ajax({
        ...
        var comment =
          "https://www.googleapis.com/youtube/v3/commentThreads?part=snippet&moderationStatus=published&order=relevance&textFormat=html&videoId=" +
          videoId +
          "&key=AIzaSyAhsb0OUjYC9-im6U3pNoks26zkjBWUtHo";
        ...
        $.ajax({
          ...
          for (i = 0; i < results.length; i++) {
            var author =
              results[i].snippet.topLevelComment.snippet.authorDisplayName;
            var authorImg =
              results[i].snippet.topLevelComment.snippet.authorProfileImageUrl;
            var comments =
              results[i].snippet.topLevelComment.snippet.textDisplay;
